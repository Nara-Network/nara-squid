...
async function getUserById(ctx: ProcessorContext, id: string): Promise<User | undefined> {
  return ctx.store.findOne(User, {
    where: { id, isTestnet: isTestnet(ctx.syncedNetwork) },
    relations: { portDeposits: true, portWithdrawalRequests: true, naraRedemptions: { activities: true } },
  });
}

async function getAllUsers(ctx: ProcessorContext): Promise<User[]> {
  return ctx.store.find(User, {
    where: { isTestnet: isTestnet(ctx.syncedNetwork) },
    relations: { portDeposits: true, portWithdrawalRequests: true, naraRedemptions: { activities: true } },
  });
}
